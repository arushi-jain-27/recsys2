...
def load_best_params(dataset_name: str, config_file: str = "best_params_config.json"):
    """Load best hyperparameters for the dataset from config file.
    
    Args:
        dataset_name: Name of the dataset
        config_file: Path to the config file containing best parameters
    ...
    try:
        if os.path.exists(config_file):
            ...
            if dataset_name in config:
                params = config[dataset_name]['best_parameters']
                alpha = params.get('alpha', DEFAULT_ALPHA)
                beta = params.get('beta', DEFAULT_BETA)
                gamma = params.get('gamma', DEFAULT_GAMMA)
                print(f"{datetime.now()}: Loaded best parameters for {dataset_name}: alpha={alpha}, beta={beta}, gamma={gamma}")
                return alpha, beta, gamma
            else:
                print(f"{datetime.now()}: No parameters found for {dataset_name} in config, using defaults")
        else:
            print(f"{datetime.now()}: Config file not found, using default parameters")
    except Exception as e:
        print(f"{datetime.now()}: Error loading config: {e}, using default parameters")
...
def compute_rfm_scores(user_id: str, baskets: List[List[int]], prices: List[List[float]], dates: List[List], 
                       alpha: float, beta: float, gamma: float) -> Dict[int, float]:
    ...
    Args:
        ...
        gamma: Weight for monetary value
    ...
    for item in item_recency.keys():
        rfm_scores[item] = alpha * normalized_recency[item] + beta * normalized_frequency[item] + gamma * normalized_revenue[item]
...
def evaluate(data_history: Dict[str, List[List[int]]], 
             ...
             alpha: float, beta: float, gamma: float) -> Dict[str, List[int]]:
    ...
    for user_id in key_set:
        ...
        rfm_scores = compute_rfm_scores(user_id, baskets, prices, dates, alpha, beta, gamma)
...
def main(argv):
    ...
    # Load best hyperparameters for this dataset
    alpha, beta, gamma = load_best_params(dataset_name)
    ...
    predicted_test = evaluate(data_history, data_prices, data_dates, keyset_test, topk, global_scores, alpha, beta, gamma)
    predicted_val = evaluate(data_history, data_prices, data_dates, keyset_val, topk, global_scores, alpha, beta, gamma)
    ...
    # Save predictions
    pred_path = f"../../predictions/{dataset_name}/rfm_new/keyset{keyset_index}.json"
