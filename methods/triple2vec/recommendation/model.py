...
class Recommender(object):
    ...
    def evaluate(self, dump=True):
        ...
        for u, items in dataValidation:
            ...
            if u in userItemFreq:
                ...
                print(f"User {u} not found in userItemFreq")
        ...
        if dump:
            # Ensure output directory exists only when writing
    ...
    def export_predictions_frequency_style(self, dataset_name, users, fold_id=0, top_k=10):
        ...
        """
        # Build path: project_root = parent of methods (same logic used elsewhere)
        from config import BASE_DIR
        project_root = os.path.dirname(BASE_DIR)
        pred_dir = os.path.join(project_root, 'predictions', dataset_name, 'triple2vec')
        os.makedirs(pred_dir, exist_ok=True)
        pred_path = os.path.join(pred_dir, f"keyset{fold_id}.json")
