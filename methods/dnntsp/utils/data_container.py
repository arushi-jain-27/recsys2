...
class SetDataset(Dataset):
    def __init__(self, history_path, future_path, keyset_path, item_embedding_matrix, key=None):
        ...
        self.total_items = data_keyset['item_num']
...
def collate_set_across_user(batch_data, total_items):
    """
    Args:
    ...
    truth_data = get_truth_data([dt[-1] for dt in data_list], total_items)
    ret.append(truth_data)

    # tensor (batch, items_total), for frequency calculation
    users_frequency = np.zeros([len(batch_data), total_items])
...
def get_data_loader(history_path, future_path, keyset_path, data_type, batch_size, item_embedding_matrix):
    ...
    data_loader = DataLoader(dataset=dataset,
                             ...
                             shuffle=False,
                             drop_last=False,
                             collate_fn=lambda x: collate_set_across_user(x, dataset.total_items))
    return data_loader
