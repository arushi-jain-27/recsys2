#!/usr/bin/env python3
"""
Hyperparameter tuning for FPMC recommendation model
"""
...
class FPMCHyperparameterTuner:
    ...
    def load_data(self):
        ...
        self.evaluator = RecommendationEvaluator(self.dataset_name, "fpmc", split="val")
        self.evaluator.predictions_path = f"../../predictions/{self.dataset_name}/fpmc/keyset0.json"
    ...
    def evaluate_predictions(self, predictions):
        temp_pred_path = f"../../predictions/{self.dataset_name}/fpmc/keyset0.json"
    ...
    def run_single_experiment(self, params):
        model = train_model(
            ...
            params["n_factor"], params["learn_rate"], params["regular"],
            params["n_neg"], params["n_epoch"],
        )
        predictions = predict(model, self.data_history, self.keyset_val)
    ...
    def print_results(self, results, best_params, best_score, metric="nDCG@5", top_n=3):
        print("\n" + "=" * 80)
        print(f"FPMC HYPERPARAMETER TUNING RESULTS - {self.dataset_name.upper()}")
        print("=" * 80)
    ...
    def save_results(self, results, best_params, best_score, metric="nDCG@5", seed=42):
        ...
        results_file = f"../../hyperparameter_results/{self.dataset_name}/fpmc.json"
    ...
    def tune(self, metric="nDCG@5", max_combinations=None, top_n=3, seed=42):
        print(f"{datetime.now()}: Starting FPMC hyperparameter tuning")
...
def main():
    parser = argparse.ArgumentParser(description="FPMC Hyperparameter Tuning")
    ...
    tuner = FPMCHyperparameterTuner(args.dataset_name)
