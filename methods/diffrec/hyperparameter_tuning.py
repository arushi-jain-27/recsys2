#!/usr/bin/env python3
"""
Hyperparameter tuning for DiffRec recommendation model
"""
...
class DiffRecHyperparameterTuner:
    def __init__(self, dataset_name):
        ...
        self.param_ranges = {
            ...
            "batch_size": [200, 400],
            "steps": [5, 10, 40],
            "noise_scale": [0.0001, 0.005, 0.01],
            "noise_min": [0.0001, 0.001, 0.005],
            "noise_max": [0.005, 0.01],
            "n_epoch": [20],
        }
    ...
    def load_data(self):
        ...
        self.evaluator = RecommendationEvaluator(self.dataset_name, "diffrec", split="val")
        self.evaluator.predictions_path = f"../../predictions/{self.dataset_name}/diffrec/keyset0.json"
    ...
    def evaluate_predictions(self, predictions):
        temp_pred_path = f"../../predictions/{self.dataset_name}/diffrec/keyset0.json"
    ...
    def run_single_experiment(self, params):
        model, diffusion, matrix = train_model(
            ...
            params["hidden"], params["lr"], params["weight_decay"], params["batch_size"],
            params["steps"], params["noise_scale"], params["noise_min"], params["noise_max"],
            params["n_epoch"],
        )
        predictions = predict(
            model, diffusion, matrix, self.keyset_val, 0, batch_size=params["batch_size"],
        )
        metrics = self.evaluate_predictions(predictions)
        del model, diffusion, matrix, predictions
    ...
    def grid_search(self, metric="nDCG@5", max_combinations=None, seed=42):
        ...
        all_combinations = []
        for combination in product(*self.param_ranges.values()):
            params = dict(zip(param_names, combination))
            if params["noise_min"] < params["noise_max"]:
    ...
    def print_results(self, results, best_params, best_score, metric="nDCG@5", top_n=3):
        print("\n" + "=" * 80)
        print(f"DIFFREC HYPERPARAMETER TUNING RESULTS - {self.dataset_name.upper()}")
        print("=" * 80)
    ...
    def save_results(self, results, best_params, best_score, metric="nDCG@5", seed=42):
        ...
        results_file = f"../../hyperparameter_results/{self.dataset_name}/diffrec.json"
    ...
    def tune(self, metric="nDCG@5", max_combinations=None, top_n=3, seed=42):
        print(f"{datetime.now()}: Starting DiffRec hyperparameter tuning")
...
def main():
    parser = argparse.ArgumentParser(description="DiffRec Hyperparameter Tuning")
    ...
    tuner = DiffRecHyperparameterTuner(args.dataset_name)
